...
const SpellingGame = () => {
  ...
  const handleLetterClick = (letter) => {
    if (userSpelling.length < currentWord.length) {
      setUserSpelling((prevSpelling) => [...prevSpelling, letter]); // Append new letter
      if (isSoundEnabled) playLetterSound();
    }
  };

  // Check if word is correct whenever userSpelling changes
  useEffect(() => {
    if (userSpelling.join('') === currentWord) {
  ...
  }, [userSpelling, currentWord, isSoundEnabled]);
  ...
  return (
    ...
    >
      <Card className="p-6 space-y-6">
        ...
        {/* Spelling Area */}
        <div
          className="min-h-16 p-2 border-2 border-dashed border-blue-300 rounded-lg flex gap-2 justify-center items-center bg-blue-50"
          onDrop={handleDrop}
        ...
        <div className="text-center text-sm text-gray-600">
          ...
          <p>Developed by Shivprakash Swami as a fun project using GenAI technologies</p>
        </div>
      </Card>
    </div>
  );
