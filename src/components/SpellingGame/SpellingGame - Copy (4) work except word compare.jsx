...
const SpellingGame = () => {
  ...
  const handleKeyPress = useCallback((e) => {
    ...
    console.log("Key pressed:", key); // Log key press for debugging
  ...
  //
  useEffect(() => {
    console.log("Adding event listener for keydown"); // Debugging log
    document.addEventListener('keydown', handleKeyPress);
    return () => {
      console.log("Removing event listener for keydown"); // Debugging log
      document.removeEventListener('keydown', handleKeyPress);
    };
  }, [handleKeyPress]); // Ensure event listener is only added once
  ...
  const handleLetterClick = (letter) => {
    if (userSpelling.length < currentWord.length) {
        setUserSpelling((prevSpelling) => [...prevSpelling, letter]); // Append the new letter
        if (isSoundEnabled) playLetterSound();
        checkWord([...userSpelling, letter]); // Check if the updated spelling is correct
    }
};
  ...
  return (
    <div 
      ...
      tabIndex="0" // Ensure component is focusable
      onFocus={() => gameContainerRef.current.focus()} // Autofocus for keyboard input
    >
